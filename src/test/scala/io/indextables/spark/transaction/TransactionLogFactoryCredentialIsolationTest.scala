...
class TransactionLogFactoryCredentialIsolationTest extends TestBase {
  ...
  /**
   * OAuth mirror: TransactionLogFactory.create() must not use a source uc.tableId for destination credential resolution
   * even when OAuth client-credential keys are present in the config.
   *
   * This mirrors the static-token isolation test above but uses a config that would activate OAuth auth mode (clientId
   * + clientSecret present). The fix that strips uc.tableId before calling resolveCredentialsOnDriver must work
   * regardless of the auth mode.
   *
   * Because the test environment does not have a live OIDC endpoint we use the mock provider class (which ignores auth
   * mode and intercepts credential calls at the CredentialProviderFactory level) and only verify that path-based (not
   * table-based) credentials were used for the destination.
   */
