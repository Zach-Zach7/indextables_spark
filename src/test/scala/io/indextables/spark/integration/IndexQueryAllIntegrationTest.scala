...
class IndexQueryAllIntegrationTest extends AnyFunSuite with TestBase with BeforeAndAfterEach {
  ...
  test("IndexQueryAllExpression should work with DataFrame operations") {
    ...
    val columnExpr = org.apache.spark.sql.indextables.ColumnCompat.column(indexQueryAllExpr)
    ...
    try {
      ...
      testQueries.foreach {
        case (queryString, description) =>
          ...
          val column = org.apache.spark.sql.indextables.ColumnCompat.column(indexQueryAllExpr)
  ...
  test("Multiple IndexQueryAllExpression in complex queries") {
    ...
    val column1 = org.apache.spark.sql.indextables.ColumnCompat.column(query1)
    val column2 = org.apache.spark.sql.indextables.ColumnCompat.column(query2)
