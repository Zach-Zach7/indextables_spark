 ...
 *
 * Reusable by the catalog and, in the future, by SQL commands (MERGE SPLITS AT TABLE ..., etc.) when they add support
 * for table-name references instead of raw storage paths.
 */
object IndexTableResolver {
   ...
   *   1. spark.indextables.aws.region (Spark session conf) 2. fs.s3a.endpoint.region (Hadoop conf) 3. aws.region (JVM
   *      system property) 4. AWS_DEFAULT_REGION (environment variable) 5. AWS_REGION (environment variable)
   */
   ...
   *
   * @param sourceTableProperties
   *   properties map from the source table (Delta/Iceberg/Parquet)
   * @param spark
   *   active SparkSession (for region auto-detection)
   * @param sourceTableLabel
   *   human-readable label for the source table, used in error messages
   * @return
   *   resolved absolute path to the companion index
   * @throws IllegalArgumentException
   *   if required properties are absent or empty
   */
  def resolveIndexPath(
    ...
    val indexRoot: String = detectedRegion
      .flatMap { region =>
   ...
   *
   * @param sourceCatalogName
   *   name of the source catalog (e.g. "unity_catalog")
   * @param sourceNamespace
   *   namespace segments within the source catalog (e.g. Array("production"))
   * @param sourceTableName
   *   table name within the namespace (e.g. "users")
   * @param spark
   *   active SparkSession
   * @return
   *   resolved absolute path to the companion index
   * @throws IllegalArgumentException
   *   if required TBLPROPERTIES are missing
   * @throws UnsupportedOperationException
   *   if the source catalog does not implement TableCatalog
   */
